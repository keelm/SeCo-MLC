 ...
 */
public class InvertedPrecision extends ValueHeuristic {
     ...
     * Constructor
     */
    public InvertedPrecision() {
    }


    private String form(boolean intMode, double d) {
        if (intMode) return "" + (int) (Math.round(d));
        else {
            String result = "" + ((int) Math.floor(d * 1000)) / 1000.0;
            while (result.length() < 5) result = result + " ";
            return result;
        }
    }
...
//		TwoClassConfusionMatrix rstats = r.getStats();
//		TwoClassConfusionMatrix predstats = r.getPredecessor().getStats();
        double P, N;
