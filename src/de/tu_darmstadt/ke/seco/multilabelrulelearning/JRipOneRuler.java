 ...
 * of each attribute and selects the condition with highest information gain: p(log(p/t)-log(P/T)).<br/> <br/> 1.2.
 * Prune phase:<br/> Incrementally prune each rule and allow the pruning of any final sequences of the antecedents;The
 * pruning metric is (p-n)/(p+n) -- but it's actually 2p/(p+n) -1, so in this implementation we simply use p/(p+n)
 * (actually (p+1)/(p+n+2), thus if p+n is 0, it's 0.5).<br/> <br/> 2. Optimization stage:<br/> after generating the
 ...
 * <p/>
 * <!-- globalinfo-end --> <p> <!-- technical-bibtex-start --> BibTeX:
 * <pre>
 ...
 * </pre>
 * <p/>
 * <!-- technical-bibtex-end --> <p> <!-- options-start --> Valid options are: <p/> <p>
