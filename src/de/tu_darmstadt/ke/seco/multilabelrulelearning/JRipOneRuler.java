...
import de.tu_darmstadt.ke.seco.models.NominalCondition;
import de.tu_darmstadt.ke.seco.models.NumericCondition;
...
/**
 * <!-- globalinfo-start --> This class implements a propositional rule learner, Repeated Incremental Pruning to Produce
 * Error Reduction (RIPPER), which was proposed by William W. Cohen as an optimized version of IREP. <br/> <br/> The
 * algorithm is briefly described as follows: <br/> <br/> Initialize RS = {}, and for each class from the less prevalent
 * one to the more frequent one, DO: <br/> <br/> 1. Building stage:<br/> Repeat 1.1 and 1.2 until the descrition length
 * (DL) of the ruleset and examples is 64 bits greater than the smallest DL met so far, or there are no positive
 * examples, or the error rate &gt;= 50%. <br/> <br/> 1.1. Grow phase:<br/> Grow one rule by greedily adding antecedents
 * (or conditions) to the rule until the rule is perfect (i.e. 100% accurate).  The procedure tries every possible value
 * of each attribute and selects the condition with highest information gain: pruningDepth(log(pruningDepth/t)-log(P/T)).<br/> <br/> 1.2.
 * Prune phase:<br/> Incrementally prune each rule and allow the pruning of any final sequences of the antecedents;The
 * pruning metric is (pruningDepth-n)/(pruningDepth+n) -- but it's actually 2p/(pruningDepth+n) -1, so in this implementation we simply use pruningDepth/(pruningDepth+n)
 * (actually (pruningDepth+1)/(pruningDepth+n+2), thus if pruningDepth+n is 0, it's 0.5).<br/> <br/> 2. Optimization stage:<br/> after generating the
 * initial ruleset {Ri}, generate and prune two variants of each rule Ri from randomized data using procedure 1.1 and
 * 1.2. But one variant is generated from an empty rule while the other is generated by greedily adding antecedents to
 * the original rule. Moreover, the pruning metric used here is (TP+TN)/(P+N).Then the smallest possible DL for each
 * variant and the original rule is computed.  The variant with the minimal DL is selected as the final representative
 * of Ri in the ruleset.After all the rules in {Ri} have been examined and if there are still residual positives, more
 * rules are generated based on the residual positives using Building Stage again. <br/> 3. Delete the rules from the
 * ruleset that would increase the DL of the whole ruleset if it were in it. and add resultant ruleset to RS. <br/>
 * ENDDO<br/> <br/> Note that there seem to be 2 bugs in the original ripper program that would affect the ruleset size
 * and accuracy slightly.  This implementation avoids these bugs and thus is a little bit different from Cohen's
 * original implementation. Even after fixing the bugs, since the order of classes with the same frequency is not
 * defined in ripper, there still seems to be some trivial difference between this implementation and the original
 * ripper, especially for audiology data in UCI repository, where there are lots of classes of few instances.<br/> <br/>
 * Details please see:<br/> <br/> William W. Cohen: Fast Effective SingleHeadRule Induction. In: Twelfth International
 * Conference on Machine Learning, 115-123, 1995.<br/> <br/> PS.  We have compared this implementation with the original
 * ripper implementation in aspects of accuracy, ruleset size and running time on both artificial data "ab+bcd+defg" and
 * UCI datasets.  In all these aspects it seems to be quite comparable to the original ripper implementation.  However,
 * we didn't consider memory consumption optimization in this implementation.<br/> <br/>
 * <pruningDepth/>
 * <!-- globalinfo-end --> <pruningDepth> <!-- technical-bibtex-start --> BibTeX:
 * <pre>
 ...
 *    title = {Fast Effective SingleHeadRule Induction},
 *    year = {1995}
 * }
 * </pre>
 * <pruningDepth/>
 * <!-- technical-bibtex-end --> <pruningDepth> <!-- options-start --> Valid options are: <pruningDepth/> <pruningDepth>
 ...
 * <pruningDepth> <!-- options-end -->
 *
 * @author Xin Xu (xx5@cs.waikato.ac.nz)
 * @author Eibe Frank (eibe@cs.waikato.ac.nz)
 * @version $Revision: 8118 $
 */
public class JRipOneRuler
    ...
    public String globalInfo() {
        ...
        return "This class implements a propositional rule learner, Repeated Incremental "
                ...
                + "the condition with highest information gain: pruningDepth(log(pruningDepth/t)-log(P/T))."
                + "\n\n"
                + "1.2. Prune phase:\n"
                + "Incrementally prune each rule and allow the pruning of any "
                + "final sequences of the antecedents;"
                + "The pruning metric is (pruningDepth-n)/(pruningDepth+n) -- but it's actually "
                + "2p/(pruningDepth+n) -1, so in this implementation we simply use pruningDepth/(pruningDepth+n) "
                + "(actually (pruningDepth+1)/(pruningDepth+n+2), thus if pruningDepth+n is 0, it's 0.5).\n\n"
    ...
    public TechnicalInformation getTechnicalInformation() {
        ...
        result.setValue(Field.TITLE, "Fast Effective SingleHeadRule Induction");
     ...
     * Valid options are: <pruningDepth>
     * <pruningDepth>
     * -F number <br>
     * The number of folds for reduced error pruning. One fold is
     * used as the pruning set. (Default: 3) <pruningDepth>
     * <pruningDepth>
     * -N number <br>
     * The minimal weights of instances within a split.
     * (Default: 2) <pruningDepth>
     * <pruningDepth>
     * -O number <br>
     * Set the number of runs of optimizations. (Default: 2)<pruningDepth>
     * <pruningDepth>
     * -D <br>
     * Whether turn on the debug mode
     * <pruningDepth>
     * -S number <br>
     * The seed of randomization used in Ripper.(Default: 1)<pruningDepth>
     * <pruningDepth>
     * -E <br>
     * Whether NOT check the error rate >= 0.5 in stopping criteria.
     * (default: check)<pruningDepth>
     * <pruningDepth>
     * -P <br>
     * Whether NOT use pruning. (default: use pruning)<pruningDepth>
     *
    ...
    /**
     * Parses a given list of options. <pruningDepth/>
     * <pruningDepth>
     * <!-- options-start -->
     * Valid options are: <pruningDepth/>
     * <pruningDepth>
     ...
     * In this class, the Information Gain (pruningDepth*[log(pruningDepth/t) - log(P/T)]) is used to
     * select an antecedent and Reduced Error Prunning (REP) with the metric
     * of accuracy rate pruningDepth/(pruningDepth+n) or (TP+TN)/(P+N) is used to prune the rule.
     */
    public class RipperRule
     ...
     The information gain of each bag is also calculated in this procedure */
         ...
         * @param useWhole  flag to indicate whether use the error rate of the whole pruning data instead of the data
         *                  covered
         */
     ...
     */
    public SingleHeadRule getBestRuleForClass(int classIndex, Instances instances) throws Exception {
        ...
        int internalClassIndex = -1;
        for (int i = 0; i < data.numClasses(); i++) {
            if (classIndex == ((ClassOrder) m_Filter).originalValue(i)) {
                internalClassIndex = i;
            }
        }
...
//		data=instances;
        ...
        if (Utils.eq(orderedClasses[internalClassIndex], 0.0)) // No data for this class
            return null;
        ...
        data = rulesetForOneClass(expFPRate, data, internalClassIndex, defDL);
...
//		}    
        ...
        if (m_Ruleset == null || m_Ruleset.size() == 0)
            return null;
        RipperRule finalRule = (RipperRule) m_Ruleset.get(0);

        //TODO: leave the jrip heuristic for next step
//		de.tu_darmstadt.ke.seco.models.SingleHeadRule secoR = getSeCoRule(finalRule, null, instances.classAttribute(),classIndex,(RuleStats) m_RulesetStats.elementAt(0), expFPRate);
        SingleHeadRule secoR = getSeCoRule(finalRule, null, instances.classAttribute(), classIndex, null, expFPRate);
        // free up memory
    ...
    static public SingleHeadRule getSeCoRule(RipperRule r, Heuristic heuristic, Attribute classAttr,
                                             int correctClassValue, RuleStats ruleStats, double expFPRate) {
        SingleHeadRule secoR = new SingleHeadRule(heuristic);
        for (int i = 0; i < r.getAntds().size(); i++) {
            Antd ant = (Antd) r.getAntds().get(i);
            Attribute attr = ant.getAttr();
            double value = ant.getAttrValue();
            Condition cond;
            if (attr.isNominal())
                cond = new NominalCondition(de.tu_darmstadt.ke.seco.models.Attribute.toSeCoAttribute(attr), value);
            else {
                cond = new NumericCondition(de.tu_darmstadt.ke.seco.models.Attribute.toSeCoAttribute(attr),
                        ((NumericAntd) ant).getSplitPoint());
                cond.setCmp(value == 0); //if value==0 then it's <=
            }
            secoR.addCondition(cond);
        }
        Condition con = new NominalCondition(de.tu_darmstadt.ke.seco.models.Attribute.toSeCoAttribute(classAttr),
                correctClassValue);
        secoR.setHead(con);
        //doesnt work as expected, leave it for next steps
        if (false) {
            final double jRipHeuristicValue = ruleStats.relativeDL(0, expFPRate, true);
            //		final double jRipHeuristicValue = ruleStats.combinedDL(expFPRate, 0);
            secoR.computeRuleValue(new Heuristic() {
                ...
                @Override
                public double evaluateRule(de.tu_darmstadt.ke.seco.models.Rule r) {
                    // TODO Auto-generated method stub
                    return -jRipHeuristicValue;
                }

                @Override
                public double evaluateConfusionMatrix(TwoClassConfusionMatrix confusionMatrix) {
                    return -jRipHeuristicValue;
                }
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        //JRip_MLRL MOD: set distribution of default class to [0,0], i.e. positive and negative class have same probabilies. this is interpreted by stacking approach as MISSING
        m_Distributions.setElementAt((double[]) new double[data.classAttribute().numValues()],
                m_Distributions.indexOf(m_Distributions.lastElement()));
    }
    ...
    protected Instances rulesetForOneClass(double expFPRate,
        ...
        while ((!stop) && hasPositive) { // Generate new rules until
            ...
            //ELM-HACK: return as soon as first rule is added
            if (!stop) {
                ...
                if (m_Debug)
                    System.err.println("One rule added: has positive? "
                            + hasPositive);
                if (m_Optimizations == 0) {
                    m_Ruleset = ruleset;
//					ELM-HACK: stop as soon as first rule found and no optimizations planned 
                    return null;
                }
            } else {
                //this happens in oneruler whenever the found first rule is the default rule. in this case, return null
                if (m_Debug)
        ...
        if (m_UsePruning) {
            for (int z = 0; z < m_Optimizations; z++) {
                ...
                while (!stop && hasPositive) {
                    ...
                    if (isResidual) {
                        ...
                        if (!stop) {
                            ...
                            //ELM-MOD: this modification (and the following one some lines after) does not guarantee you that the resulting first rule is the same as for finishing training, because we skip the point later where we check each rule whether to remove it from the ruleset (reduceDL). however, we assume that the first rule is never removed because it should be the best one, and practice (on emotions) confirms this. reason for still doing it: it's faster!
                            if (z == m_Optimizations - 1 && ruleset.size() == 1) {
                                m_Ruleset = ruleset;
                                return null;
                            }
                        } else {
                        ...
                        //ELM-MOD:
                        if (z == m_Optimizations - 1 && position == 0) {
                            m_Ruleset = ruleset;
                            return null;
                        }
                    }
                ...
                Object firstRule = null;
//				if(finalRulesetStat.getRulesetSize()!=0){
//					firstRule=finalRulesetStat.getRuleset().get(0);
//				}
                finalRulesetStat.reduceDL(expFPRate, m_CheckErr);
//				if(finalRulesetStat.getRulesetSize()!=0 && firstRule != finalRulesetStat.getRuleset().get(0)){
//					System.err.println("That's bad"); //ELM-MOD: that means that the first rule is removed, so our modification would produce here a different outcome 
//				}
                if (m_Debug) {
...
//			return data; 
    ...
    public static void main(String[] args) {
        runClassifier(new JRipOneRuler(), args);
    }
}
